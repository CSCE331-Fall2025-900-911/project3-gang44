...
export const getBobaRecommendation = (temp, weatherCode) => {
  // Temperature in Fahrenheit
  if (temp >= 80) {
    return {
      drink: 'Mango Green Tea',
      reason: `It's ${Math.round(temp)}°F! Perfect weather for a refreshing fruit tea`,
      emoji: '☀️'
    };
  } else if (temp >= 65) {
    return {
      drink: 'Honey Milk Tea',
      reason: `Nice ${Math.round(temp)}°F weather calls for a sweet and smooth classic`,
      emoji: '🌤️'
    };
  } else if (temp >= 50) {
    return {
      drink: 'Brown Sugar Milk Tea',
      reason: `At ${Math.round(temp)}°F, enjoy something warm and comforting`,
      emoji: '🍂'
    };
  } else if (weatherCode >= 71 && weatherCode <= 77) {
    return {
      drink: 'Hot Taro Milk Tea',
      reason: `It's snowing! Warm up with a cozy hot drink`,
      emoji: '❄️'
    };
  } else if (weatherCode >= 51 && weatherCode <= 67) {
    return {
      drink: 'Jasmine Milk Tea',
      reason: `Rainy day? A soothing tea is just what you need`,
      emoji: '🌧️'
    };
  } else {
    return {
      drink: 'Thai Milk Tea',
      reason: `At ${Math.round(temp)}°F, treat yourself to something special`,
      emoji: '✨'
    };
  }
};
        ...
        if ('geolocation' in navigator) {
          navigator.geolocation.getCurrentPosition(
            async (position) => {
              ...
              const rec = getBobaRecommendation(temp, weatherCode);
            ...
            (err) => {
              ...
              setRecommendation({
                ...
                reason: 'A timeless favorite for any occasion',
                emoji: '🥤'
              });
              setLoading(false);
            }
